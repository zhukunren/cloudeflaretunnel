...
class ModernTunnelManager(tk.Tk):
    ...
    def _build_modern_ui(self):
        ...
        self._create_icon_button(tool_buttons, "⬇", self._check_update_cloudflared, "下载或更新")
        ...
        control_card = self._create_modern_card(right_panel, "控制台", icon="🚀")
        control_card["card"].pack(fill=tk.X, pady=(0, 15))
        control_section = control_card["content"]
        ...
        action_row1 = tk.Frame(control_section, bg=Theme.BG_CARD)
        action_row1.pack(fill=tk.X, pady=(12, 0))

        self._create_outline_button(action_row1, "✏ 编辑配置", self._edit_selected_config)
        self._create_outline_button(action_row1, "🧪 诊断测试", self._test_tunnel)

        action_row2 = tk.Frame(control_section, bg=Theme.BG_CARD)
        action_row2.pack(fill=tk.X, pady=(10, 0))

        self._create_outline_button(action_row2, "📊 守护状态", self._show_supervisor_status)
        self._create_outline_button(action_row2, "📂 打开目录", self._open_config_dir)
    ...
    def refresh_tunnels(self):
        ...
        path = self.cloudflared_path.get().strip()
        if not path:
            self.tunnel_list.set_tunnels([], "尚未配置 cloudflared", "请先在顶部工具栏配置 cloudflared。")
